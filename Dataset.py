...
def prepare_datasets(model_config):
    ...
    if model_config['local_run']:  # If running on local machine, mini dataset is all in one folder
        ...
        if model_config['dataset'] in ['both', 'LibriSpeech']:
            ...
            voice_train_dirs.extend(glob(model_config['librispeech_data_root'] + 'Voice/train-clean-360/**/', recursive=True))
            ...
            mix_train_dirs.extend(glob(model_config['librispeech_data_root'] + 'Mixed/train-clean-360/**/', recursive=True))
