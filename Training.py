...
def cfg():
    model_config = {"model_base_dir" : "checkpoints", # Base folder for model checkpoints
                    ...
                    'cache_size' : 32, # Number of audio excerpts that are cached to build batches from
...
def test(model_config, audio_list, model_folder, load_model):
    ...
    [mix_context, voice],  queue, input_ph = Input.get_multitrack_input(sep_output_shape[1:], model_config["batch_size"], name="input_batch", input_shape=sep_input_shape[1:])
...
def train(model_config, sup_dataset, model_folder, unsup_dataset=None, load_model=None):
    ...
    freq_bins = model_config["num_fft"] / 2 + 1  # Make even number of freq bins
    disc_input_shape = [model_config["batch_size"], int(freq_bins - 1), model_config["num_frames"],1]  # Shape of discriminator input
    ...
    # Placeholders and input normalisation
    mix_context,voice = Input.get_multitrack_placeholders(sep_output_shape, sep_input_shape, "sup")
    mix = Input.crop(mix_context, sep_output_shape)
    mix_norm, mix_context_norm, voice_norm = Input.norm(mix), Input.norm(mix_context), Input.norm(voice)

    if unsup_dataset != None:
        mix_context_u,voice_u = Input.get_multitrack_placeholders(sep_output_shape, sep_input_shape, "unsup")
        mix_u = Input.crop(mix_context_u, sep_output_shape)
        mix_norm_u, mix_context_norm_u, voice_norm_u = Input.norm(mix_u), Input.norm(mix_context_u), Input.norm(voice_u)
    ...
    if unsup_dataset != None:
        separator_voice_norm_u = separator_func(mix_context_norm_u, reuse=True)
        separator_voice_u = Input.denorm(separator_voice_norm_u)
        mask_loss_u = tf.reduce_mean(tf.square(mix_u - separator_voice_u))
    mask_loss = tf.reduce_mean(tf.square(mix - separator_voice))
    ...
    if unsup_dataset != None:
        ...
        # Define real and fake inputs for discriminator - if separator output and dsicriminator input shapes do not fit perfectly, we will do a centre crop and only discriminate that part
        ...
        L_u = - tf.reduce_mean(voice_disc_fake)  # WGAN based loss for separator (L_u in paper)
    ...
    voice_disc_vars = Utils.getTrainableVariables("voice_disc")
    ...
    while run:
        ...
        if unsup_dataset != None:
            ...
            _, _unsup_summaries, _sup_summaries = sess.run(
                ...
                feed_dict={mix_context: sup_batch[0], voice: sup_batch[1],
                           mix_context_u: unsup_batches[0], voice_u:unsup_batches[1]}
            )
    ...
    save_path = saver.save(sess, os.path.join(model_config["model_base_dir"], model_folder, model_folder), global_step=int(_global_step))
...
_       '''
        ...
        train_unsup,_ = Datasets.getCHiME3('train_unsup')
        train_sup = list(Datasets.getCHiME3('train_sup'))
        validation = list(Datasets.getCHiME3('validation'))
        test = list(Datasets.getCHiME3('test'))
        ...
        dataset = dict()
        dataset["train_sup"] = train_sup
        dataset["train_unsup"] = train_unsup
        ...
        dataset["train_sup"] = zip(dataset["train_sup"][0], dataset["train_sup"][1])
        dataset["valid"] = zip(dataset["valid"][0], dataset["valid"][1])
        dataset["test"] = zip(dataset["test"][0], dataset["test"][1])
