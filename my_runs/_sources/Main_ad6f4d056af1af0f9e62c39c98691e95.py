    ...
    while True:
        try:
            ...
            for i in range(model_config['BATCH_SIZE']):
                voice_est = af.spectrogramToAudioFile(voice_est_mag[i, :, :].T, model_config['N_FFT'], model_config['FFT_HOP'],
                                                      phase=mixed_phase[i, :, :].T)
                voice = af.spectrogramToAudioFile(voice_mag[i, :, :].T, model_config['N_FFT'], model_config['FFT_HOP'],
                                                  phase=mixed_phase[i, :, :].T)
...
def do_experiment(model_config, experiment_id):
    ...
    mixed_mag = mixed[0][:, :, 1:, 0]  # Yet more hacking to get around this tuple problem
    mixed_phase = mixed[0][:, :, 1:, 1]
    voice_mag = voice[0][:, :, 1:, 0]
