...
def prepare_datasets(model_config):
    ...
    y_train = get_dataset(model_config['data_root']+'train_sup/Voice',
                          ...
                          model_config['PATCH_HOP'],
                          model_config['N_PARALLEL_READERS'])
    train_data = zip_datasets(x_train, y_train, model_config['N_SHUFFLE'], model_config['BATCH_SIZE'], shuffle=True)
    ...
    y_val = get_dataset(model_config['data_root']+'validation/Voice',
                        ...
                        model_config['PATCH_HOP'],
                        model_config['N_PARALLEL_READERS'])
    val_data = zip_datasets(x_val, y_val, model_config['N_SHUFFLE'], model_config['BATCH_SIZE'], shuffle=True)
    ...
    y_test = get_dataset(model_config['data_root']+'test/Voice',
                         ...
                         model_config['PATCH_HOP'],
                         model_config['N_PARALLEL_READERS'])
    test_data = zip_datasets(x_test, y_test, model_config['N_SHUFFLE'], model_config['BATCH_SIZE'], shuffle=False)
