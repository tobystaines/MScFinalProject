...
class BatchGen_Paired:
    '''
    Generates two matched batches of mixture and voice, both in the shape [batch_size, freqs, time, 1]
    so that the ground truth output for mixture[x,:,:,:] is voice[x,:,:,:].
    '''
    def __init__(self,
                 model_config,
                 dataset,
    ...
    def get_batch(self):
        ...
        time_padding = int((self.options["input_shape"][2] - self.options["output_shape"][2])/2) # Amount of temporal context in the input on each side
        ...
        for sample_num in range(self.options["batch_size"]): # For each sample consisting of mix, acc voice...
            ...
            voice[sample_num, :, :, 0] = Utils.pad_freqs(cache_item[1][:,start_idx_input:stop_idx_input], self.options["output_shape"][1:3])
