...
def get_multitrack_placeholders(shape, input_shape=None, name=""):
    ...
    :return: List of multitrack placeholders for mixture and voice
    '''
...
def get_multitrack_input(shape, batch_size, name="", input_shape=None):
    ...
    :param input_shape: Shape of voice magnitudes
    :param batch_size: Number of samples in each batch
    :param name: How to name the placeholders
    :return: [List of mixture and voice placeholders, random shuffle queue, symbolic batch sample from queue]
    '''
    m, v = get_multitrack_placeholders(shape, input_shape=input_shape)
...
def crop(tensor, target_shape):
    ...
    output = tensor
    if diff[1] > 0:
        output = output[:,int(diff[1]):-int(diff[1]),:,:]
    if diff[2] > 0:
        output = output[:,:,int(diff[2]):-int(diff[2]),:]
    return output

def load_and_enqueue(sess, model_config, queue, enqueue_op, input_ph, sample_list):
    '''
